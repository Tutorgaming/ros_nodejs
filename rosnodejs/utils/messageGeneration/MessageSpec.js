...
var RosMsgSpec = function () {
  ...
  _createClass(RosMsgSpec, [{
     ...
     * @private
     */
    value: function _extractRelevantMessage(fileContents) {
      ...
      switch (this.type) {
        case SRV_REQUEST_TYPE:
          {
            var divider = lines.indexOf(MSG_DIVIDER);
            lines = lines.slice(0, divider);
            break;
          }
        case SRV_RESPONSE_TYPE:
          {
            var _divider = lines.indexOf(MSG_DIVIDER);
            lines = lines.slice(_divider + 1);
            break;
          }
        case ACTION_GOAL_TYPE:
          {
            var _divider2 = lines.indexOf(MSG_DIVIDER);
            lines = lines.slice(0, _divider2);
            break;
          }
        case ACTION_RESULT_TYPE:
          {
            var divider1 = lines.indexOf(MSG_DIVIDER) + 1;
            var divider2 = lines.indexOf(MSG_DIVIDER, divider1);
            lines = lines.slice(divider1, divider2);
            break;
          }
        case ACTION_FEEDBACK_TYPE:
          {
            var _divider3 = lines.indexOf(MSG_DIVIDER);
            _divider3 = lines.indexOf(MSG_DIVIDER, _divider3 + 1);
            lines = lines.slice(_divider3 + 1);
            break;
          }
        default:
          break;
    ...
    key: '_extractFields',
    value: function _extractFields(content) {
...
var SrvSpec = function (_RosMsgSpec2) {
  ...
  function SrvSpec(msgCache, packageName, messageName, type) {
    ...
    _this6.request = new MsgSpec(msgCache, packageName, messageName + 'Request', SRV_REQUEST_TYPE, null, _this6.fileContents);
    _this6.response = new MsgSpec(msgCache, packageName, messageName + 'Response', SRV_RESPONSE_TYPE, null, _this6.fileContents);
    return _this6;
  }
...
var ActionSpec = function (_RosMsgSpec3) {
  ...
  function ActionSpec(msgCache, packageName, messageName, type) {
    ...
    _this7.goal = new MsgSpec(msgCache, packageName, messageName + 'Goal', ACTION_GOAL_TYPE, null, _this7.fileContents);
    _this7.result = new MsgSpec(msgCache, packageName, messageName + 'Result', ACTION_RESULT_TYPE, null, _this7.fileContents);
    _this7.feedback = new MsgSpec(msgCache, packageName, messageName + 'Feedback', ACTION_FEEDBACK_TYPE, null, _this7.fileContents);
