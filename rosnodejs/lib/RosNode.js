  ...
  _createClass(RosNode, [{
    ...
    value: function _setupExitHandler() {
      ...
      var exitImpl = function exitImpl() {
        ...
        Object.keys(this._subscribers).forEach(function (topic) {
          promises.push(_this14.unsubscribe(topic));
        });

        Object.keys(this._publishers).forEach(function (topic) {
          promises.push(_this14.unadvertise(topic));
        });

        Object.keys(this._services).forEach(function (service) {
          promises.push(_this14.unadvertiseService(service));
        });
        ...
        var shutdownServers = function shutdownServers() {
          return Promise.all([shutdownServer(_this14._slaveApiServer, 'slaveapi'), shutdownServer(_this14._tcprosServer, 'tcpros')]);
        };
